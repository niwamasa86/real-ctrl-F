...
const evalCalculation = (function (String, pow, max) {
	'use strict'

	function getDecimalPartLength (numberString) {
		const result = /\.\d+$/.exec(numberString)

		return result ? result[0].length - 1 : 0
	}

	const calcMultiplyOrDivide = (function (push) {
		return function calcMultiplyOrDivide (expression) {
			let multiplyOrDivide = /\*(-?(?:\d+(?:\.\d+)?|\.\d+))|\/(-?(?:\d+(?:\.\d+)?|\.\d+))|(-?(?:\d+(?:\.\d+)?|\.\d+))/g, multiply = [], divide = [], number, token, i, len
	...
	function calcDyadicOperator (matched, number1, operator, number2) {
		let decimalPart = /\.\d+$/, powerNumber1, powerNumber2, result
	...
	return function evalCalculation (expression) {
		let multiplyOrDivide = /-?(?:\d+(?:\.\d+)?|\.\d+)(?:[*/]-?(?:\d+(?:\.\d+)?|\.\d+))+/g,
...
module.exports = evalCalculation
